...
**Solution:**  _ordinal character variable_ 
...
**Solution:**  _character vector, help to visualise the count of hashtags_ 
...
**Solution:**  _non-numeric data type, because the timing can be break into date(yy/mm/dd) and time of the user interaction, which it's easier to analyse by treating it as non-numeric data _ 
...
**Solution:**  _non-numeric data type _ 
...
**Solution:**  _nominal character type_ 
...
**Solution:**  _three related variables may be average time spent on social media per day, age and gender, where average time spent is continuous numeric type with the unit of minutes, whereas age is discrete numeric type and gender is nominal non-numerical type_ 
...
print(combined)
# when combined two different types, a common data type will be used to handle both vector, in this case, character type.
```
...
```{r, eval=TRUE,echo=TRUE}
mixed_data <- list(numerical_vector = c(10,20,30), character_vector = c("red", "green", "blue"), logical_vector = c(TRUE,FALSE,TRUE))
